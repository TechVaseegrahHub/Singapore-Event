  ...
  return (
    <section id="how-to-book" className="bg-[#4757a6] py-12 px-4 md:py-20 md:px-10">
      <div className="max-w-7xl mx-auto">
        ...
        <div
          ref={scrollRef}
          className="flex overflow-x-auto pb-8 gap-4 snap-x snap-mandatory md:grid md:grid-cols-4 md:pb-0 md:gap-6 scrollbar-hide justify-items-center"
        >
          {STEPS.map((item) => (
            <div
              key={item.id}
        ...
        {/* Dynamic Dot Indicator — mobile only */}
          ...
          {STEPS.map((_, i) => (
            <button
              ...
              className={`rounded-full transition-all duration-300 ease-in-out focus:outline-none ${
                i === activeIndex
                  ? "bg-[#f57f20] w-6 h-3 shadow-[0_0_8px_rgba(245,127,32,0.7)]"
